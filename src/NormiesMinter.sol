 ...
 */
contract NormiesMinter is Ownable, Lifebuoy {
    ...
    /**
     * @notice Mints a token with server-signed data. Supports delegate.xyz v2 for cold wallet delegation.
    ...
    function mint(
        ...
        // Verify caller is minter or a delegate.xyz v2 delegate
        if (msg.sender != minter) {
            require(
                DELEGATE_REGISTRY.checkDelegateForAll(msg.sender, minter, "")
                    || DELEGATE_REGISTRY.checkDelegateForContract(msg.sender, minter, address(normies), ""),
                NotMinterOrDelegate()
            );
        }
    ...
    function batchMint(
        ...
        // Verify caller is minter or a delegate.xyz v2 delegate (once for the batch)
        if (msg.sender != minter) {
            require(
                DELEGATE_REGISTRY.checkDelegateForAll(msg.sender, minter, "")
                    || DELEGATE_REGISTRY.checkDelegateForContract(msg.sender, minter, address(normies), ""),
