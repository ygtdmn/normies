...
/**
 * @title NormiesRendererV4
 * @author Normies by Serc (https://x.com/serc1n)
 * @author Smart Contract by Yigit Duman (https://x.com/yigitduman)
 * @dev Update: V4 Renderer with NormiesCanvas support
 */
contract NormiesRendererV4 is INormiesRenderer, Ownable, Lifebuoy {
    ...
    function tokenURI(uint256 tokenId) external view override returns (string memory) {
        ...
        bytes memory originalData = storageContract.getTokenRawImageData(tokenId);
        uint256 pixelCount = _countPixels(originalData);
        bytes memory imageData;

        if (transformed) {
            bytes memory customLayer = transformStorageContract.getTransformedImageData(tokenId);
            imageData = _composite(originalData, customLayer);
        } else {
        ...
        bytes memory part2 = abi.encodePacked(
            _buildAttributes(traits, tokenId),
            ",",
